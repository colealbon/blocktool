    ...
    return txidToRawTransaction(txid)
        .then(function(rawtransaction) {
            ...
            return rawtransaction.vout.map(function(voutItem) {
                voutItem.destroy_start = rawtransaction.time;
...
const slow_vinToInputDetail = function(vin) {
    ...
    vinGuard(vin);
    const inputDetailVal = [];
    const inputDetail = vin
        .map(function(vinitem) {
            return vinItemToInputDetailItem(vinitem).then(function(
                outputdetail) {
                outputdetail.map(function(voutitem) {
                    if (voutitem !== undefined) {
                        logger.silly({
                            'slow_vinToInputDetail <-> voutitem': voutitem
                        });
                        inputDetailItemGuard(voutitem);
                        inputDetailVal.push(voutitem);
                        return voutitem;
                    }
                }, {
                    concurrency: 1
    ...
    if (inputDetail.length === vin.length) {
        logger.silly({
            'vinToInputDetail -> inputDetail': inputDetail
        });
        return inputDetailVal;
    }
};

const vinToInputDetail = slow_vinToInputDetail;
module.exports.vinToInputDetail = vinToInputDetail;

const arrGuard = function(a) {
    if (Object.prototype.toString.call(a) !== '[object Array]') {
        throw new TypeError("expected Array but received " + typeof(a));
    }
};
...
const blockHeaderGuard = function(blockheader) {
    ...
    const blockHeaderLense = lenses.Lens.objectLens,
        o = void intGuard(blockHeaderLense('time').run(blockheader)
            .get());
    return o;
};
...
const guessGuard = function(guessObj) {
    ...
    const guessLense = lenses.Lens.objectLens,
        lowcount = guessLense('lowcount').run(guessObj).get(),
        highcount = guessLense('highcount').run(guessObj).get(),
        targettime = guessLense('targettime').run(guessObj).get();
    intGuard(lowcount);
    intGuard(highcount);
    intGuard(targettime);
};
...
const vinItemGuard = function(vinitem) {
    ...
    const vinItemLense = lenses.Lens.objectLens,
        ...
        coinbase = vinItemLense('coinbase').run(vinitem).get(),
...
const inputDetailItemGuard = function(inputDetailItem) {
    ...
    const inputDetailItemLense = lenses.Lens.objectLens,
        value = inputDetailItemLense('value').run(inputDetailItem).get(),
        n = inputDetailItemLense('n').run(inputDetailItem).get(),
        txid = inputDetailItemLense('txid').run(inputDetailItem).get(),
        destroy_start = inputDetailItemLense('destroy_start').run(
            inputDetailItem).get();
    if (txid !== undefined) {
        numGuard(value);
        intGuard(n);
...
const rawTransactionGuard = function rawTransactionGuard(rawtransaction) {
    logger.silly({
        'rawTransactionGuard <- ': {
    ...
    const rawTransactionLense = lenses.Lens.objectLens,
        txid = rawTransactionLense('txid').run(rawtransaction).get(),
        blocktime = rawTransactionLense('time').run(rawtransaction)
        .get(),
        vin = rawTransactionLense('vin').run(rawtransaction).get();
    str64Guard(txid);
    intGuard(blocktime);
    vinGuard(vin);
};
