...
const slow_dateRangeToTxid = function dateRangeToTxid(daterange) {
    ...
    typecheck.dateRangeGuard(daterange);
    return dateRangeToBlockRange(daterange).then(function(blockrange) {
        return blockRangeToBlockCount(blockrange)
            .map(function(blockcount) {
                return blockCountToBlockhash(blockcount);
            })
...
const txidToTransactionSummary = function txidToTransactionSummary(
    txid) {
    logger.debug({
        "txidToTransactionSummary( <-": {
            "txid": txid
        }
    });
    //typecheck.str67Guard(txid);
    return txidToTransactionSignature(txid)
    .then(function(transactionsignature) {
        let vout_count = transactionsignature.vout.length
        let vin_count = transactionsignature.inputdetail.length
        let vout_value = transactionsignature.vout.map(function(vout) {
                    return parseInt((vout.value * 1e8) + .5)
                }).reduce(function(a, b) { return a + b})
        let vin_value = transactionsignature.inputdetail.map(function(inputitem) {
                return (inputitem) ? ((parseInt(inputitem.value) * 1e8) + .5) : 0
            }).reduce(function(a, b) { return a + b})

        let satoshi_seconds_destroyed = (transactionsignature.inputdetail) ?
            transactionsignature.inputdetail.map(function(inputitem) {
                    let satoshi = (inputitem) ? parseInt((inputitem.value * 1e8) + .5) : 0;
                    let seconds_destroyed = (inputitem) ? (inputitem.destroy_stop - inputitem.destroy_start) : 0;
                    let satoshi_seconds_destroyed = satoshi * seconds_destroyed;
                    return satoshi_seconds_destroyed;
                }).reduce(function(a, b) { return a + b}) :
                0;
        return {
            "txid": txid,
            "vout_count": vout_count,
            "vin_count": vin_count || 0,
            "vout_value": vout_value,
            "vin_value": vin_value || 0,
            "satoshi_seconds_destroyed": satoshi_seconds_destroyed || 0
        };
    })
};
module.exports.txidToTransactionSummary =
    txidToTransactionSummary;
...
const vinItemToInputDetailItem = function vinItemToInputDetailItem(vinItem) {
    logger.silly({'vinItemToInputDetailItem <- vinItem': vinItem});
    if (vinItem.txid === undefined) {
        ...
        return txidToVout(vinItem.txid)
            .then(function(vout) {
                logger.silly({'vinItemToInputDetailItem <-> vout': vout});
                return vout.map(function(voutitem) {
                    if (voutitem.n === vinItem.vout) {
                        logger.silly({'vinItemToInputDetailItem -> voutitem': voutitem});
                        typecheck.voutItemGuard(voutitem);
                        return voutitem;
                    }
                }, {
                    concurrency: 1
...
const vinToInputDetail = function vinToInputDetail(vin) {
    logger.silly({'vinToInputDetail <- vin': vin});
