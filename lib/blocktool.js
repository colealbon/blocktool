...
const txidToTransactionSummary = function txidToTransactionSummary(
    ...
    .then(function(transactionsignature) {
        ...
        let vin_value = transactionsignature.inputdetail.map(function(inputitem) {
                    return parseInt((inputitem.value * 1e8) + .5)
                }).reduce(function(a, b) { return a + b})
        let satoshi_seconds_destroyed = transactionsignature.inputdetail.map(function(inputitem) {
                    let satoshi = parseInt((inputitem.value * 1e8) + .5)
                    let seconds_destroyed = inputitem.destroy_stop = inputitem.destroy_start
    ...
    //     return transactionsignature.vout
    // };
};
