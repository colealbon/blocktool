...
const str67Guard = function(x) {
    ...
    if (x.length !== 67) {
        throw new TypeError(
            "expected 67 character string but received " +
            x.length + " " + x);
    }
};
module.exports.str67Guard = str67Guard;
...
const inputDetailItemGuard = function(inputdetailItem) {
    ...
    const inputdetailItemLense = lenses.Lens.objectLens,
        ...
        destroy_start = inputdetailItemLense(
            'destroy_start').run(
            inputdetailItem).get(),
        destroy_stop = inputdetailItemLense(
            'destroy_stop').run(
            inputdetailItem).get();
    if (txid !== undefined) {
        numGuard(value);
...
const dateRangeGuard = function(daterange) {
    logger.debug({
        'dateRangeGuard <-': {
            'daterange': daterange
        }
